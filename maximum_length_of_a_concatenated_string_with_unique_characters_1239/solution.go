...
func explore(arr []string, memo map[string]int, freq []int) int {
	...
	for i := range arr {
		...
		// remove the choice at arr[i] and explore using modern slices.Delete
		nextArr := slices.Delete(slices.Clone(arr), i, i+1)
		maxLen = max(maxLen, explore(nextArr, memo, freq))
